import supabase from "../supabaseClient";

function AuthButtons() {
  const handleOAuthLogin = async (provider) => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider,
      options: {
        redirectTo: window.location.origin,
      },
    });

    if (error) {
      alert(`[${provider} 로그인 실패]: ${error.message}`);
    }
  };

  return (
    <>
      <button
        type="button"
        onClick={() => handleOAuthLogin("google")}
        className="flex justify-center items-center gap-2 w-full h-[40px] py-[10px] px-[12px] rounded-md bg-white text-black"
      >
        <svg
          width="20"
          height="20"
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
          <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
          <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
          <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
          <path d="M1 1h22v22H1z" fill="none" />
        </svg>
        <p>Continue with Google</p>
      </button>
      <button
        type="button"
        onClick={() => handleOAuthLogin("kakao")}
        className="flex justify-center items-center gap-2 w-full h-[40px] py-[10px] px-[12px] rounded-md bg-[#FEE500] text-black"
      >
        <svg
          width="20"
          height="20"
          viewBox="0 0 24 24"
          fill="currentColor"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M12.03 2C6.07 2 1.25 5.88 1.25 10.38c0 2.53 1.63 4.77 4.13 6.22l-1.13 3.43a.5.5 0 0 0 .73.6l4.14-2.3c.91.17 1.87.26 2.86.26 5.96 0 10.78-3.88 10.78-8.38S18 2 12.03 2z" />
        </svg>
        <p>Continue with Kakao</p>
      </button>
    </>
  );
}

export default AuthButtons;